...
export class GraphManager {
  ...
  makeXAxis() {
    this.graph.addEdge(
      ...
      { x: this.endPointXX, y: this.endPointXY },
      null
    );
  ...
  makeYAxis() {
    this.graph.addEdge(
      "yAxis",
      { x: this.originX, y: this.originY },
      { x: this.endPointYX, y: this.endPointYY },
      null
    );
  ...
  showPortfolio(data) {
    ...
    while (count <= this.timespan) {
      ...
      let val = this.dataConverter.getPortfolioValueAtGivenDate(date.getTime());
      this.bars.push({ date: date, value: val, id: count });
      count++;
    }
    this.bars.forEach((bar, ind, arr) => {
      !(ind % this.spacing) && ind < arr.length - 5
        ? setTimeout(this.makeBarAndLabel, this.getInterval() * 50, bar)
        : this.makeBar(null);
    });
  }
